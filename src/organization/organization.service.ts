...
export class OrganizationService {
    ...
    async create(organizationDTO: CreateOrganizationDTO, userid: string) {
      ...
      async organizationUserLink(
        ...
        const link = await this.organizationUserModel.findOne({ user:user._id, organization: organization._id});
        if (!link) {
    ...
    async checkOwnerOrganization(
      ...
      async deleteOrganization(
        ...
        return await this.organizationUserModel.deleteMany({organization: link.organization});
      }
      ...
      async leaveOrganization(
        ...
        const link = await this.organizationUserLink(await this.checkOrganizationById(id), user);
        if (link.roleUser == roleUserEnum.admin){
      ...
      async fireUser(
        ...
        const organization = await this.checkOrganizationById(id);
        const link = await this.organizationUserModel.findOne({ user:admin._id, organization: organization});
        await this.checkOwnerOrganization(link);
        await this.organizationUserModel.deleteOne({organization:organization, user: await this.userService.checkUserById(user)});
        return await link.deleteOne();
      }

      async checkOrganizationById(
        id:string
