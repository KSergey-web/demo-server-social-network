...
import { NotificationUserLink, NotificationUserLinkDocument } from './schemas/notification-user.schema';
...
export class NotificationService {
    constructor(
      ...
      async getNotification(userId: string){
        const links = await this.notificationUserLinkModel.find({user:userId}).populate('notification').exec();
        let nots = [];
        let remains;
        for(let i = 0; i< links.length; ++i){
          ...
          organization: await this.organizationService.checkOrganizationById((links[i].notification as Notification).organization as string),
          team: await this.teamService.getTeamById((links[i].notification as Notification).team as string),
          task: await this.taskService.getTaskById((links[i].notification as Notification).task as string),
      ...
      async create(task: TaskDocument, phase: phaseEnum) {
        ...
        consoleOut(phase,'ph');
        const check = await this.notificationModel.findOne({task:task._id, phase}).exec();
        if (check) return;
        ...
        task.users.forEach(async user=>{
          ...
          const createdNotificationUser = new this.notificationUserLinkModel(
            link,
          );
      ...
      async markAllAsReaded(userId: string){
        const links = await this.notificationUserLinkModel.find({user:userId}).exec();
      ...
      async getNotReaded(userId: string):Promise<number>{
        const links = await this.notificationUserLinkModel.find({user:userId, isReaded:false}).exec();
        return links.length;
      }
}
