...
import { FileResAndBuffer, FileResource, FileResourceDocument } from './schemas/group.schema';
...
export class FileResourceService {
  ...
  async getDefaultForUser(): Promise<FileResAndBuffer>{
    const fileRes: FileResource = {
      ...
      name: (new Date().toJSON().slice(0, 19) + path.parse(this.defImageForUser).ext).replace(/\s/g, '').replace(/[:]/g, "")
    }
    const buffer = fs.readFileSync(`./assets/originals/${this.defImageForUser}`);
  ...
  async saveImageAvarage(buffer: Buffer, fileRes: FileResource): Promise<boolean> {
    if (this.isImage(fileRes.mimetype)) {
      await fs.promises.writeFile(
        `./assets/average/${fileRes.name}`,
        (await sharp(buffer)
          .resize(200, 300).toBuffer())
      );
      return true
    }
    else return false
  ...
  async getFileIfImage(resFile: FileResource): Promise<FileResAndBuffer> {
    if (this.isImage(resFile.mimetype)) {
      try {
        const buffer = (await this.getFile(resFile._id)).buffer.toString('base64');
  ...
  async getFilesIfImage(resFiles: Array<FileResource>):Promise<FileResAndBuffer[]> {
  ...
  async getAvatar(fileId:string, type: avatarTypeEnum): Promise<{buffer:string}> {
  ...
  async getAverageImage (fileRes: FileResource) {
    try{
    const bitmap = await fs.promises.readFile(`./assets/average/${fileRes.name}`);
    return ('data:'+fileRes.mimetype+';base64,')+bitmap.toString('base64')
    }
    catch(e){
      consoleOut(e);
  ...
  async getOriginalImage (fileRes: FileResource){
    const bitmap = await fs.promises.readFile(`./assets/originals/${fileRes.name}`);    
    return ('data:'+fileRes.mimetype+';base64,')+bitmap.toString('base64')
  }
